...
def plot_by_task(net, task, epochs):
    ...
    testloader = DataLoader(testset, batch_size=1, pin_memory=True, shuffle=True, num_workers=opt.num_workers)
    ...
    with torch.no_grad():
        for ([img_name], degrad_patch, clean_patch) in tqdm(testloader):
            ...
            for i in range(inter.shape[0]):
                v = get_frequency_distribution(inter[i], norm=False)
                    
                low.append(v[0])
                high.append(np.sum(v[1:]))
...
opt.cuda = 2
opt.output_path = 'output/encoder_type_ViT_encoder_dim_64_crop_test_imgs_size_128_de_type_denoising_15_denoising_25_denoising_50_deraining_test_de_type_denoising_bsd68_15_denoising_bsd68_25_denoising_bsd68_50_deraining/'
